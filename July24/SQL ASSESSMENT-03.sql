...
SELECT p.name, COUNT(v.visit_id) AS num_visits
FROM Pets p
JOIN Visits v ON p.pet_id = v.pet_id
GROUP BY p.name
...
-- 11. Update the fee for visit_id 105 to 350 (Safe mode-friendly)
UPDATE Visits
SET fee = 350.00
WHERE visit_id = 105;

-- 12. Delete all visits made before Feb 2024 (using subquery for safe mode)
DELETE FROM Visits
WHERE visit_id IN (
